...
class TestEmailSubject:
    def test_medicine_format(self):
        subject = build_email_subject(Category.MEDICINE, 2026, 8)
        assert subject == "[RPA] 의약품 품목 갱신 신청기한 알림 (신청 기한 2026년 08월)"

    def test_device_format(self):
        subject = build_email_subject(Category.DEVICE, 2026, 2)
        assert subject == "[RPA] 의료기기 품목 갱신 신청기한 알림 (신청 기한 2026년 02월)"


class TestEmailBody:
    def test_medicine_body_contains_table(self):
        items = load_medicine_csv(DATA_DIR / "의약품갱신일정.csv")
        alert = find_medicine_alerts(items, date(2026, 5, 19))
    ...
    def test_device_body_contains_table(self):
        ...
        alert = find_device_alerts(items, date(2026, 2, 1))
        assert alert is not None
        body = build_email_body(alert)
        assert "<table" in body
